...
extern uint16_t govWinMs;   // SD governor measurement window and decision interval, /control?govWinMs
extern uint16_t govWrites;  // governor quality writes this clip - the churn metric
...
extern bool fpsPriority; // trade JPEG quality to hold the requested rate, see sdGovernor()
...
const frameStruct frameData[] = {
  ...
  {"HD", 1280, 720, 30, 3, 1, 56},    // 52 -> 56 on 7 Sep 2026: HD JOINS 1280X960 ON ROUTE B at a 2112 line, so 88e6 / (2112 x 744) = 56.00 against the 52.20 the 80 MHz tree gave at HTS 2060 - VSYNC-counted 56.006, still clean on both the whole-frame and per-band gates, and a 25 s clip delivered 54.3 (BOARD_TESTING 38.41-44). Route B engages at a request of 49; 1-48 keep route A and differ from the old tier ONLY in the line, which also ends the 1280-wide frame stretch HTS 2060 showed (53.885 counted against 57.42 predicted there, against 56.006 against 56.00 here). Requests 54-56 program IDENTICAL registers, VTS already at its 744 floor - the QVGANARROW 144/147 trap again. Still owed: sweep.csv regeneration, route B below the ceiling (49-53 program VTS 810/793/778/763/748 and no frame has been looked at on any of them), and a real sustained-rate pass | previously: KEEPS its sensor ceiling of 52, and fpsPriority is what makes that honest - see the note below frameData. A sustained-rate pass on 7 Sep 2026 showed 52 delivering 49.1 and even 47 sliding 47.0 -> 45.9 as the evening light went, but the governor was PINNED at boost 4 for every one of those clips: it was out of room, not out of ideas. With GOV_MAX_BOOST_FPS it can keep trading quality for the rate. Storage-bound at ~19ms per 85KB frame | PY260 | measured 31.9 at HTS_FLOOR, 25.3 before it | tuned 52.06 MEASURED 27 Aug, exact 12-52 | exposure-first sweep 28 Aug: 95ms @10fps, 399ms @1fps (PIXCLK walk, VTS held 1968 below 19fps) | re-swept 2 Sep 2026 lit q10: register tier identical; 52 delivers 51.4 at governor boost 2, 51 delivers 48.5 at boost 1 - storage time per 75 KB frame (17 of 19 ms), scene-dependent, not the sensor (BOARD_TESTING 26)
  ...
  {"QSXGA", 2560, 1920, 2, 4, 1, 6},  // 7 -> 6 on 7 Sep 2026, SUSTAINED-RATE pass: 7 delivered 6.7 at 99% busy and 112ms of storage per 480KB frame, 6 delivered 6.0 exactly twice at boost 0. NOTE the frames were 480KB against the 2 Sep lit reference 354KB, so this was measured in a DIMMER room and is the conservative answer  // measured 2.0 @ 59% busy | tuned 7.00 MEASURED 27 Aug (full array, HTS 2844 x2); q6 frames 764KB - see the budget badge | swept 1-7 28 Aug all exact; 140ms @7fps, 952ms @1fps (PIXCLK 11.73) | re-swept 2 Sep 2026 lit q10: register tier identical, 7.0 exact at 354 KB frames, boost 0, busy 54%
  ...
  {"QVGANARROW", 320, 240, 20, 2, 1, 147}, // 7 Sep 2026: a sustained-rate pass lowered this to 144 and it was REVERTED the same hour, for two reasons. (1) 144 AND 147 PROGRAM THE SAME REGISTERS - VTS 264, sensor 147.10fps - because the tuner is already at its VTS floor here, so a request of 144 runs the sensor at 147.10 and lets the frame timer decimate. Lowering the ceiling to 144 changes no timing at all; it only moves the slider stop. (2) THE SHORTFALL IS NOT STORAGE. The short rungs carry 4-12ms of MONITORING time - waiting inside esp_camera_fb_get - against 2ms of storage, where the clean rung had 0ms. At this size max exposure is 7ms, and the room that day ran ~35% larger frames than the 2 Sep lit reference, so the sensor was light-starved and delivery went erratic: 144.0 twice in one run, 134.3 in the next, and a request of 134 delivered 67.3, a clean halving. A light limit is not a pipeline limit and no ceiling fixes it. Re-measure this size in a LIT room before touching it  // window 704x512 -> 2x2 -> ISP in 352x256 -> pre-scale 320x240, scaler 1:1 | 27% of array width, 26% of height | MEASURED 3 Sep 2026: 147.059 VSYNC-counted at VTS 264, against 39.474 for full-frame QVGA | max exposure 6.7ms, so this size needs real light
  {"HDV2", 1280, 720, 30, 3, 1, 56}   // 720p on the DATASHEET's own 720p readout: 1440 array rows instead of the driver's 1472, binned to exactly the 720 it emits, ISP offset zero. Identical output, identical framing - the 16 binned rows HD gives up here are the ones HD's own offset discards anyway. Worth one thing only: VTS can sit at 738 instead of 744, so 88e6 / (2112 x 738) = 56.46 against HD's 56.00. THAT IS THE WHOLE DIFFERENCE, and it appears at THREE of 56 requests (54, 55, 56) - everywhere else the two sizes program byte-identical registers, because rows read do not enter fps and the window only ever sets how low VTS may go (measured: 1440 rows at VTS 744 counted 54.845 against 54.825 at 1472 rows). Its value is that a request of 56 regains an overdrive margin HD does not have: HD runs the sensor at exactly 56.00 for a request of 56, which is the "ceiling requests deliver ~1% low" case applyTunedTiming documents | window y 256..1695, yOff 0, scaler off, HTS 2112, route B - see applyTightRows() and HDV2_VTS_FLOOR
};

// CEILINGS AND fpsPriority, 7 Sep 2026. Read this before changing any maxTunedFPS above.
// A sustained-rate pass (fps_sustain_descend.sh) asked what each size DELIVERS at its ceiling. Nine
// of eleven reproduced exactly across independent runs; two did not, and the split falls on one
// number in the closeAvi block:
//   GOVERNOR BOOST 0 AT THE CEILING -> repeatable. The six sizes that keep their ceilings, plus QHD
//     and QSXGA once lowered, all delivered their request EXACTLY, twice each, in two runs.
//   GOVERNOR BOOST PINNED AT ITS CAP -> the governor had run out of room, and delivery then tracked
//     the SCENE. Over ~45 minutes of failing light, 1280X960 at a request of 38 gave 37.9, 37.8,
//     37.6 then 34.8 as frames grew 106 -> 115KB; HD at 47 gave 47.0, 47.0, 46.6, 45.9, 46.2.
//     Same code, same registers (checked), same board. Only the room changed.
//
// THE FIX WAS HEADROOM, NOT A NEW MECHANISM: those two were pinned at boost 4 the whole time. With
// GOV_MAX_BOOST_FPS they can keep trading, so HD and 1280X960 keep their SENSOR ceilings (52, 41)
// and fpsPriority makes the storage path meet them.
//
// QHD AND QSXGA STAY LOWERED (9->8, 7->6) AND fpsPriority DOES NOT HELP THEM, for a reason worth
// knowing: the push arm triggers on demand crossing GOV_PUSH_PCT of the SD budget, and at QSXGA 7
// demand is only ~75% of budget while the size still misses the rate - the missing time is
// monitoring and buffering, which the governor cannot see. It never pushes, so no amount of boost
// headroom reaches it. That is CLAUDE.md open item 2, and lowering GOV_PUSH_PCT is NOT the quick
// fix: GOV_RELAX_PCT is derived from it through one step's effect on demand, and that effect is
// itself unsettled - 1.5x at QSXGA (28 Aug) against a MEASURED 2.9% per step at 1280X960 q14-q20
// (7 Sep, quality_buys_rate.sh). Re-measure the step at QSXGA before touching either threshold.
//
// QVGANARROW keeps 147: its shortfall is MONITORING time, not storage - see its row.
...
// HDV2: 720p read the way the datasheet's own 720p mode reads it. Table 2-1 specifies "cropping
// 2592x1944 to 2560x1440 ... 1296x728 with dummy", ie the 720 rows it emits plus 8 dummy lines,
// where the driver's generic 16:9 window reads 1472 array rows, bins to 736, and has the ISP
// offset throw 16 of them away. HDV2 reads 1440 rows and offsets nothing, so the frame is shorter
// by 8 binned rows and VTS can come down with it. Framing is IDENTICAL to HD - the rows given up
// are the ones HD's offset already discards, which is why the window is centred at 256 rather
// than kept at HD's 240. See applyTightRows(). Index 30, appended: never insert above a custom size
#define FS_HDV2 (FRAMESIZE_INVALID + 5)       // index 30
#define FS_HDV2_BASE FRAMESIZE_HD             // same 16:9 window, then the rows are cut
#define HDV2_ROWS 1440                        // array rows read, against HD's 1472
#define HDV2_Y_START 256                      // HD reads 240..1711 and outputs 256..1695; this
                                              // reads exactly what it outputs
// EIGHTEEN blanking lines, not the eight the binned rule (rows/2 + 8) and the datasheet's 728
// both predict. MEASURED 7 Sep 2026 (BOARD_TESTING 38.41): 744/740/738 all count their predicted
// rate exactly, 736 is marginal (clean in three readings, collapsed in three others), 734 and 732
// deliver EXACTLY HALF the rate with complete and perfectly legible frames, and 728 delivers
// nothing at all. Only a VSYNC count catches the half-rate mode; every image gate passes it
#define HDV2_VTS_FLOOR 738
